...
df = df.loc[(df['grp'] == 'BRANCH') & (df['region'] == 'Core') & ((df['metric'] == 'BR_MISP_RETIRED_ALL_BRANCHES') | (df['metric'] == 'BR_INST_RETIRED_ALL_BRANCHES'))]
...
dfInstr = dfAggAVG.xs('BR_INST_RETIRED_ALL_BRANCHES', level='metric').rename(index=str, columns={"value": "instr"}).stack()
dfMiss = dfAggAVG.xs('BR_MISP_RETIRED_ALL_BRANCHES', level='metric').rename(index=str, columns={"value": "miss"}).stack()
...
ax.set_ylabel('Misses per Kilo Branches')
...
fig.savefig('/home/flav/Mestrado/MHaJoL/data/out/plots/mpkb.png',  bbox_inches='tight')
# fig.show()
fig.clf()
# print('1')
# # plt.close()
