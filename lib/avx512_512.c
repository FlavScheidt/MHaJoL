...
const uint64_t permTB[512] = {0x0706050403020100ull,
...
inline void vivid512Generate(column_orders * c_orders)
{
	...
	// __m512i thresholdVector = _mm512_set1_epi32(VCUCKOO_MAX_TRY);
...
inline int vivid512LookUp(__m512i key)
{
	...
	hash1 = _mm512_or_si512(hash1, hash2);
	hash1 = _mm512_cmpeq_epi32(hash1, zeroVector);

	found = _mm512_mask2int(_mm512_movepi32_mask(hash1));
...
int vivid512Join(column_customer * c_customer, column_orders * c_orders)
{
	...
	{
		keys = _mm512_maskload_epi32(&customer[i], oneVector);
